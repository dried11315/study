...
//分页查询
const load = ()=>{
  request.get('/prouser/selectPage',{
    params:{
...
const handleDelete = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/prouser/delete/' + id).then(res => {
      if (res.code === '200') {
        load()
...
//新增
const add = ()=>{
  request.post('/prouser/add', data.form).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
...
//更新
const update = ()=>{
  request.put('/prouser/update', data.form).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      data.formVisible = false
      load()
